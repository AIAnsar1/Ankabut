...
from core.bigarray import BigArray
...
from core.convert import getConsoleLengthm, getUnicode
from core.data import conf, kb, logger, queries
from core.dicts import DUMP_REPLACEMENTS
from core.enums import CharsetType, DBMS, Expected, Payload
from core.exception import AnkabConnectionException, AnkabMissingMandatoryOptionException, AnkabNoneDataException, AnkabUnsupportedFeatureException
from core.settings import CHECK_ZERO_COLUMNS_THRESHOLD, CURRENT_DB, METADB_SUFFIX, NULL, PLUS_ONE_DBMSES, UPPER_CASE_DBMSES
from request import inject
from support.hash import attackDumpedTable
from support.pivotdumptable import pivotDumpTable
from pkg import six
from pkg.six.moves import zip as _zip
...
class Entries(object):
    ...
    def dumpTable(self, foundData=None):
        ...
        if conf.db is None or conf.db == CURRENT_DB:
            if conf.db is None:
                warnMsg = "missing database parameter. Ankabut is going "
            ...
            if ',' in conf.db:
                ...
                raise AnkabMissingMandatoryOptionException(errMsg)
        ...
        if conf.tbl:
            ...
            if len(kb.data.cachedTables) > 0:
                ...
                raise AnkabNoneDataException(errMsg)
            else:
                return
        ...
        for tbl in tblList:
            ...
            try:
                    ...
                    if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.DB2, DBMS.DERBY, DBMS.ALTIBASE, DBMS.MIMERSQL):
                        ...
                        if not (isTechniqueAvailable(Payload.TECHNIQUE.UNION) and kb.injection.data[Payload.TECHNIQUE.UNION].where == Payload.WHERE.ORIGINAL):
                            ...
                            if Backend.isDbms(DBMS.MSSQL) and not conf.forcePivoting:
                                ...
                                count = inject.getValue(query, blind=False, time=False, expected=Expected.INT, charsetType=CharsetType.DIGITS)
                ...
                if not kb.data.dumpedTable and isInferenceAvailable() and not conf.direct:
                    ...
                    query = agent.whereQuery(query)
                    count = inject.getValue(query, union=False, error=False, expected=Expected.INT, charsetType=CharsetType.DIGITS)
                    lengths = {}
                    entries = {}
                ...
                if len(kb.data.dumpedTable) == 0 or (entriesCount == 0 and kb.permissionFlag):
                    ...
                    kb.data.dumpedTable["__infos__"] = {"count": entriesCount,"table": safeSQLIdentificatorNaming(tbl, True), "db": safeSQLIdentificatorNaming(conf.db)}
            ...
            except AnkabConnectionException as ex:
    ...
    def dumpAll(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            ...
            raise AnkabUnsupportedFeatureException(errMsg)
