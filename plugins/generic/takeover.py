...
from core.common import Backend, getSafeExString, isDigit, isStackingAvailable, openFile, readInput, runningAsAdmin
from core.data import conf, kb, logger
from core.enums import DBMS, OS
from core.exceptions import (
    AnkabFilePathException, 
    AnkabMissingDependence, 
    AnkabMissingMandatoryOptionException, 
    AnkabMissingPrivileges,
    AnkabNotVulnerableException,
    AnkabSystemException,
    AnkabUndefinedMethod,
    AnkabUnsupportedDBMSException,
)
from tkw.tkw import Abstraction
from takeover.icmpsh import ICMPsh
from tkw.metasploit import Metasploit
from tkw.registry import Registry
...
class Takeover(Abstraction, Metasploit, ICMPsh, Registry):
    ...
    def osCmd(self):
        if isStackingAvailable() or conf.direct:
            ...
            raise AnkabNotVulnerableException(errMsg)
    ...
    def osPwn(self):
        ...
        if tunnel == 2:
            ...
            if not isAdmin:
                ...
                raise AnkabMissingPrivileges(errMsg)
            ...
            try:
                ...
                raise AnkabMissingDependence(errMsg)
                ...
                if not goUdf:
                    ...
                    if setupSuccess is not True:
                        if Backend.isDbms(DBMS.MYSQL):
                            ...
                            raise AnkabFilePathException(msg)
    ...
    def osSmb(self):
        ...
        if not Backend.isOs(OS.WINDOWS):
            ...
            errMsg += "relay attack"
            raise AnkabUnsupportedDBMSException(errMsg)
