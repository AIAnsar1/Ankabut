...
from core.convert import encodeHex, getUnicode
from core.data import conf, kb, logger, queries
from core.dicts import DB2_PRIVS, FIREBIRD_PRIVS, INFORMIX_PRIVS, MYSQL_PRIVS, PGSQL_PRIVS
from core.enums import CharsetType, DBMS, Expected, Payload, Fork
from core.exceptions import AnkabNoneDataException, AnkabUserQuitException
from core.settings import CURRENT_USER, PLUS_ONE_DBMSES
from core.threads import getCurrentThreadData
from request import inject
from support.hash import attackCachedUsersPasswords
from support.hash import storeHashesToFile
                    ...
                    if Backend.getIdentifiedDbms() in (DBMS.INFORMIX, DBMS.VIRTUOSO):
                        ...
                        if not isNumPosStrValue(count):
                            if Backend.isDbms(DBMS.MSSQL):
                                fallback = True
                                count = inject.getValue(query.replace("master.dbo.fn_varbintohexstr", "sys.fn_sqlvarbasetostr"), union=False, error=False, expected=Expected.INT, charsetType=CharsetType.DIGITS)
                            elif Backend.isDbms(DBMS.MYSQL):
                                fallback = True
                                count = inject.getValue(query.replace("authentication_string", "password"), union=False, error=False, expected=Expected.INT, charsetType=CharsetType.DIGITS)
